...
The recurring refresh deliberately does **not** force-refresh the per-user "current year" view on every tick, only the much cheaper current *month*. Server-wide "current year" stats decompose into per-month buckets and cache completed months indefinitely, so refreshing that view only ever re-fetches the still-open month - cheap. Per-user stats have no such decomposition: a "current year" fetch always re-queries the *entire* year-to-date range (by September, ~9 months of history) for that one user, every time, with no partial caching. Force-refreshing that on a 15-minute schedule for every user, forever, is expensive enough on its own to keep a self-hosted Tracearr/Emby instance busy continuously - so it only happens once, at startup; afterwards it's left to its normal 1-hour cache TTL, refreshed only when an actual visitor looks at it. The warmup and recurring refresh also can't overlap: if a pass is still running when the next is due, the next one is skipped rather than stacking on top of it (this is logged as `[cache-warmup] Skipping ... a previous warmup/refresh pass is still running`).
