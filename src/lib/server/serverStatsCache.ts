...
import { getAllTracearrPlaybackActivity } from './tracearr';
...
/** Whether the given time range is still "in progress" (needs periodic re-fetching). */
export function isCurrentPeriod(timeRange: TimeRange): boolean {
    const now = new Date();
    return (
...
/**
 * The set of period strings (e.g. "2025", "3-2026") that are fully in the
 * past and therefore safe to cache indefinitely / warm once at startup.
 */
export function getCompletedPeriods(): string[] {
    const now = new Date();
    const currentYear = now.getFullYear();
    return getAvailableTimeRanges()
        .map((opt) => opt.value)
...
/**
 * Get server-wide stats for a period, serving from cache when possible.
 * Completed periods are cached indefinitely; the current, in-progress
 * period is refreshed after CURRENT_PERIOD_CACHE_TTL.
 *
 * `forceRefresh` bypasses a still-valid cache entry — used by the
 * scheduled background refresh so the current period is kept warm without
 * waiting for a real user request to hit an expired entry.
 */
export async function getOrComputeServerStats(periodParam: string, forceRefresh = false): Promise<ServerStats> {
    const timeRange = parseTimeRange(periodParam);
    const cacheKey = timeRangeToString(timeRange);
    const currentPeriod = isCurrentPeriod(timeRange);

    const cached = cachedStatsMap.get(cacheKey);
    if (!forceRefresh && cached && (!currentPeriod || Date.now() - cached.time < CURRENT_PERIOD_CACHE_TTL)) {
        return cached.stats;
    }

    const stats = await computeServerStats(periodParam);
